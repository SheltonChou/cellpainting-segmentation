...
|---|---|---|---|
| `project_paths.py` | Central repository-relative paths and environment-variable overrides | environment | path objects |
...
| `baseline_cellpose_rxrx1.py` | Cellpose object-count reference on RxRx1 channel 1 | RxRx1 images | `results/baseline_rxrx1/` |
...
| `multi_seed.py` | Eight-seed repeated training | DSB2018 | `results/multi_seed/` |
...
| `routeB_instance_eval.py` | Final object-level checkpoint evaluation and bootstrap | DSB2018, checkpoints | `results/routeB_instance_eval/` |
| `density_vs_performance.py` | Held-out density/error analysis | DSB2018, checkpoint | `results/density_analysis/` |
| `channel_intensity_analysis.py` | RxRx1 channel-intensity diagnostic | RxRx1 | `results/channel_analysis/` |
| `aug_feature_impact.py` | Feature-change diagnostic under augmentation | RxRx1 | `results/aug_feature_impact/` |
| `cell_type_bias_analysis.py` | Cell-type and density diagnostic | RxRx1, checkpoint | `results/cell_type_bias/` |
| `statistical_tests.py` | Paired Wilcoxon comparisons | archived multi-seed JSON | `results/statistical_tests.json` |
...
| `evaluate_combined.py` | Earlier combined-checkpoint cross-check | DSB2018, checkpoints | `results/combined_eval_unified.json` |

For the final reported object-level results, use `routeB_instance_eval.py` and
