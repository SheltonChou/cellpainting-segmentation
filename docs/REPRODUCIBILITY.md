...
70/15/15 split after a seed-specific shuffle. Within a seed, configurations use
...
seed order. With eight non-zero paired differences, the minimum attainable
two-sided exact p-value is 0.0078125.

## 6. Pseudo-labelling

Run the volume study and iterative confidence-threshold experiment:
...
three retraining rounds. Both scripts start from the geometric-augmentation
checkpoint, so `results/checkpoints/aug_geometric_only.pth` must exist.
...
This compares geometric augmentation alone with geometric-plus-pseudo and
geometric-plus-intensity-plus-pseudo configurations. It requires the geometric
checkpoint and the archived pseudo-label selection record under
`results/pseudo_label/`.

## 8. Final instance-level evaluation
...
- matching: Hungarian one-to-one assignment;
- F1 IoU thresholds: 0.50-0.95;
- ranked mask AP: 101-point interpolated precision-recall;
- uncertainty: 2,000 paired image-level bootstrap resamples.
